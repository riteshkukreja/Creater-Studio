...
const IBusType_1 = __webpack_require__(2);
class EventBus {
    constructor(uuid, debug) {
        this._uuid = null;
        this.debug = false;
        this.dom = $("<div/>");
...
exports.StudioEventBus = new EventBus(IBusType_1.IBusType.GLOBAL, false);
exports.FilterEventBus = new EventBus(IBusType_1.IBusType.FILTER);
exports.LayerEventBus = new EventBus(IBusType_1.IBusType.LAYER);
exports.ToolEventBus = new EventBus(IBusType_1.IBusType.TOOL, true);
...
var IBusType;
(function (IBusType) {
    IBusType["GLOBAL"] = "global-eventbus";
    IBusType["LAYER"] = "layer-eventbus";
    IBusType["FILTER"] = "filter-eventbus";
    IBusType["TOOL"] = "tool-eventbus";
    IBusType["BRUSH"] = "brush-eventbus";
})(IBusType = exports.IBusType || (exports.IBusType = {}));
...
class Manager {
    ...
    bootstrap(queue) {
        this._queue = queue;
        this.eventBus = EventBus_1.StudioEventBus; //EventBusManager.singleton().get(this._queue.toString());
    }
    add(item) {
        try {
            const uuid = Helper_1.uuidGenerator();
            item.setId(uuid);
            ...
            throw new InvalidArgException_1.InvalidArgException("argument must be of type item");
        }
    }
    update(item) {
        try {
            ...
            if (uuid !== null && this.itemMap.has(uuid)) {
                ...
                throw new ItemNotFoundException_1.ItemNotFoundException("argument doesn't exists");
            }
        }
        catch (e) {
            throw new InvalidArgException_1.InvalidArgException("argument must be of type item");
    ...
    delete(uuid) {
        ...
        if (item !== undefined) {
            ...
            throw new ItemNotFoundException_1.ItemNotFoundException(`item with id ${uuid} doesn't exists`);
        }
    }
    get(uuid) {
        const item = this.itemMap.get(uuid);
...
(function (ManagerEvents) {
    ManagerEvents["ADD"] = "add";
    ManagerEvents["UPDATE"] = "update";
    ManagerEvents["REMOVE"] = "delete";
})(ManagerEvents = exports.ManagerEvents || (exports.ManagerEvents = {}));
...
exports.EventMessageItem = EventMessageItem;
...
const IBusType_1 = __webpack_require__(2);
const EventBus_1 = __webpack_require__(1);
const ColorLoader_1 = __webpack_require__(12);
class BrushManager extends Manager_1.Manager {
    constructor() {
        super(...arguments);
    ...
    bootstrap() {
        super.bootstrap(IBusType_1.IBusType.BRUSH);
        /** my events */
        EventBus_1.StudioEventBus.subscribe(BrushManagerEvents.DRAW_BRUSH, (event, { position, context }) => {
            const color = ColorLoader_1.ColorLoaderModule.getSelected();
            this.drawBrush(position, color, context);
            EventBus_1.StudioEventBus.publish(BrushManagerEvents.DRAW_POINT_SUCCESS, position);
        });
        EventBus_1.StudioEventBus.subscribe(BrushManagerEvents.DRAW_BRUSH_END, (event, { position, context }) => {
            this.reset();
        });
    }
    static singleton() {
    ...
    setSelected(id) {
        try {
            ...
            EventBus_1.StudioEventBus.publish(BrushManagerEvents.BRUSH_SELECTED, this.selectedBrush);
        }
        catch (e) {
            throw e;
        }
    ...
    drawBrush(position, color, context) {
...
BrushManager.self = null;
exports.BrushManager = BrushManager;
var BrushManagerEvents;
(function (BrushManagerEvents) {
    BrushManagerEvents["DRAW_BRUSH"] = "brushmanager:draw-brush";
    BrushManagerEvents["DRAW_POINT_SUCCESS"] = "brushmanager:draw-point-success";
    BrushManagerEvents["DRAW_BRUSH_END"] = "brushmanager:draw-brush-end";
    BrushManagerEvents["BRUSH_SELECTED"] = "brushmanager:brush-selected";
})(BrushManagerEvents = exports.BrushManagerEvents || (exports.BrushManagerEvents = {}));
...
class ColorLoader extends ControlSwatch_1.ControlSwatch {
    ...
    drawHistoryItem(pos, color) {
        const swatch = $('<span/>', { class: 'color' });
        swatch.css("background-color", color.toString());
        swatch.data("pos", pos.toLocaleString());
    ...
    drawColorGroup() {
        const colorGroup = $("<div/>", { class: 'color-group' });
        const titleSpan = $("<span/>", {
            class: 'group-title',
            text: 'All'
        });
        const colorPalete = $("<div/>");
    ...
    drawAlphaGroup(title) {
        const brushGroup = $("<div/>", { class: 'swatch-group' });
        const titleSpan = $("<span/>", {
            class: 'group-title',
            text: title
        });
        /** Brush Size content */
        const brushPalete = $("<div/>");
        /** Brush minimum size icon */
        const sizeSliderMinInfo = $("<span/>", {
            class: 'slider-min',
            text: '1'
        });
        /** Brush Size slider */
        ...
        sizeSlider.on("input", (e) => {
            this.selectedColor.Alpha = parseFloat(sizeSlider.val() || '1');
            console.log(this.selectedColor.Alpha);
        });
        /** Brush maximum size icon */
        const sizeSliderMaxInfo = $("<span/>", {
            class: 'slider-max',
            text: '100'
        });
    ...
    setSwatch() {
        ...
        return (e) => {
            ...
            if (swatch) {
                const pos = swatch.data("pos");
                const type = swatch.data("type");
                if (type === "history") {
    ...
    drawSwatchItems() {
        const colorPalete = $("<div/>");
        const alphaGroup = this.drawAlphaGroup("Opacity");
        colorPalete.append(alphaGroup);
        if (this.history.length > 0) {
            const frequentlyUsedColorGroup = this.drawHistoryGroup();
            colorPalete.append(frequentlyUsedColorGroup);
        }
        const allColorGroup = this.drawColorGroup();
        colorPalete.append(allColorGroup);
        return colorPalete;
    }
    drawDetailItem() {
        const selectedDetail = $("<i/>", {
    ...
    bootstrap(parent) {
        this.selectedColor = this.colors[0] || new Color_1.Color(0, 0, 0, 100);
        this.draw(parent);
    }
    getSelected() {
...
exports.ColorLoaderModule = new ColorLoader();
...
const IBusType_1 = __webpack_require__(2);
const Manager_1 = __webpack_require__(3);
class LayerManager extends Manager_1.Manager {
    bootstrap() {
        super.bootstrap(IBusType_1.IBusType.LAYER);
    }
    static singleton() {
        if (this.self === null) {
    ...
    updateLayer(id, canvas, context, width, height) {
        try {
    ...
    setSelected(id) {
        try {
            ...
            EventBus_1.StudioEventBus.publish(LayerManagerEvents.LAYER_SELECTED, this.selectedLayer);
        }
        catch (e) {
            throw e;
        }
    }
    getAllLayers(callback) {
        try {
            const itr = this.iterator();
            let item = itr.next();
            while (!item.done) {
    ...
    setVisibility(id, visible) {
        try {
            ...
            this.update(layer);
            EventBus_1.StudioEventBus.publish(LayerManagerEvents.VISIBILITY_CHANGED, layer.clone());
        }
        catch (e) {
            throw e;
        }
    ...
    setAlpha(id, alpha) {
        try {
            ...
            this.update(layer);
            EventBus_1.StudioEventBus.publish(LayerManagerEvents.ALPHA_CHANGED, layer.clone());
        }
        catch (e) {
            throw e;
        }
...
LayerManager.self = null;
exports.LayerManager = LayerManager;
var LayerManagerEvents;
(function (LayerManagerEvents) {
    LayerManagerEvents["LAYER_SELECTED"] = "layermanager:layer-selected";
    LayerManagerEvents["ALPHA_CHANGED"] = "layermanager:alpha-changed";
    LayerManagerEvents["VISIBILITY_CHANGED"] = "layermanager:visibility-changed";
})(LayerManagerEvents = exports.LayerManagerEvents || (exports.LayerManagerEvents = {}));
...
const IBusType_1 = __webpack_require__(2);
const EventBus_1 = __webpack_require__(1);
class ToolManager extends Manager_1.Manager {
    constructor() {
        super(...arguments);
        this.selectedTool = null;
    }
    bootstrap() {
        super.bootstrap(IBusType_1.IBusType.TOOL);
        /** my events */
        EventBus_1.StudioEventBus.subscribe(ToolManagerEvents.EXECUTE_TOOL, (event, { position, color, context, width, height }) => {
            this.executeTool(position, color, context, width, height)
                .then(() => EventBus_1.StudioEventBus.publish(ToolManagerEvents.EXECUTE_TOOL_SUCCESS, position))
                .catch((err) => console.error(err));
        });
        EventBus_1.StudioEventBus.subscribe(ToolManagerEvents.TOOL_RESET, (event, data) => {
    ...
    executeTool(position, color, context, width, height) {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.selectedTool !== null) {
                yield this.selectedTool.draw(position, color, context, width, height);
            }
        });
    }
    setSelected(id) {
        try {
            ...
            EventBus_1.StudioEventBus.publish(ToolManagerEvents.TOOL_SELECTED, this.selectedTool);
        }
        catch (e) {
            throw e;
        }
...
ToolManager.self = null;
exports.ToolManager = ToolManager;
var ToolManagerEvents;
(function (ToolManagerEvents) {
    ToolManagerEvents["EXECUTE_TOOL"] = "toolmanager:execute-tool";
    ToolManagerEvents["TOOL_RESET"] = "toolmanager:tool-reset";
    ToolManagerEvents["EXECUTE_TOOL_SUCCESS"] = "toolmanager:execute-tool-success";
    ToolManagerEvents["TOOL_SELECTED"] = "toolmanager:tool-selected";
})(ToolManagerEvents = exports.ToolManagerEvents || (exports.ToolManagerEvents = {}));
...
class ControlSwatch {
    ...
    updateSwatches() {
        ...
        /** Selected Brush details */
        const selectedDetail = this.drawSwatchItems();
    ...
    swatchView() {
        this.swatchHolder = $("<div/>", {
            class: 'swatches hide'
        });
        const swatchItem = this.drawSwatchItems();
...
exports.ControlSwatch = ControlSwatch;
...
Object.defineProperty(exports, "__esModule", { value: true });
const Exception_1 = __webpack_require__(8);
class ItemNotFoundException extends Exception_1.Exception {
    constructor(message) {
...
class Board {
    ...
    initialize() {
        this.currentBoard = $("<canvas/>", { id: 'currBoard' });
        this.currContext = this.currentBoard.get(0).getContext('2d');
        this.backBoard = $("<canvas/>");
    ...
    updateAllCanvas(id) {
        ...
        LayerManager_1.LayerManager.singleton().getAllLayers((layer, i) => {
            if (i < id) {
    ...
    createImageBoard(id, src, width, height, visible, opacity) {
        return $("<img/>", {
            src: src,
            'data-id': id,
    ...
    putPoint(context, e) {
        if (e.clientX !== undefined && e.clientY !== undefined) {
            EventBus_1.StudioEventBus.publish(BoardEvents.MOUSE_DOWN, new Position_1.Position(e.clientX, e.clientY - 50));
            EventBus_1.StudioEventBus.publish(ToolManager_1.ToolManagerEvents.EXECUTE_TOOL, {
                position: new Position_1.Position(e.clientX, e.clientY - 50),
                color: ColorLoader_1.ColorLoaderModule.getSelected(),
                context: context,
                width: this.width,
                height: this.height
            });
    ...
    handleMouseEvents() {
        ...
        this.currentBoard.on("mousemove", (e) => {
            ...
            if (e.clientX !== undefined && e.clientY !== undefined) {
                this.mouseLocationViewer.text(new Position_1.Position(e.clientX, e.clientY - 50).toString());
            }
        });
        this.currentBoard.on("mouseup", (e) => {
            ...
            if (self.backContext !== null && self.backContext !== undefined) {
                self.backContext.beginPath();
                if (e.clientX !== undefined && e.clientY !== undefined) {
                    EventBus_1.StudioEventBus.publish(ToolManager_1.ToolManagerEvents.TOOL_RESET, {
                        position: new Position_1.Position(e.clientX, e.clientY - 50),
    ...
    clearAllCanvas() {
        if (this.currContext !== null && this.backContext !== null) {
            this.currContext.clearRect(0, 0, parseInt(this.currentBoard.attr('width') || this.width.toString()), parseInt(this.currentBoard.attr('height') || this.height.toString()));
            this.backContext.clearRect(0, 0, parseInt(this.backBoard.attr('width') || this.width.toString()), parseInt(this.backBoard.attr('height') || this.height.toString()));
        }
    ...
    saveLayer() {
        if (this.selectedLayer !== null && this.currContext !== null && this.backContext !== null) {
            this.selectedLayer.update(this.backBoard[0], this.backContext, parseInt(this.backBoard.attr('width') || this.width.toString()), parseInt(this.backBoard.attr('height') || this.height.toString()));
            LayerManager_1.LayerManager.singleton().update(this.selectedLayer);
        }
    }
    applyLayer(layer) {
        if (this.currContext !== null) {
            //currContext.clearRect(0, 0, currentBoard.attr('width'), currentBoard.attr('height'));
            layer.applyDrawing(this.currContext, parseInt(this.currentBoard.attr('width') || this.width.toString()), parseInt(this.currentBoard.attr('height') || this.height.toString()));
        }
    }
    getImage() {
        const dummyCanvas = $("<canvas/>");
        const dummyContext = dummyCanvas.get(0).getContext('2d');
    ...
    bootstrap(parent) {
        ...
        /** Layer Events */
        EventBus_1.StudioEventBus.subscribe(LayersPanel_1.LayersPanelEvents.LAYER_SELECTED, (event, data) => {
            try {
                if (data == null) {
                    this.selectedLayer = null;
                    return;
        ...
        EventBus_1.StudioEventBus.subscribe(LayerManager_1.LayerManagerEvents.VISIBILITY_CHANGED, (event, data) => {
        ...
        EventBus_1.StudioEventBus.subscribe(LayerManager_1.LayerManagerEvents.ALPHA_CHANGED, (event, data) => {
            const id = data.getId();
        ...
        /** Brush event */
        EventBus_1.StudioEventBus.subscribe(ToolManager_1.ToolManagerEvents.EXECUTE_TOOL_SUCCESS, (event, data) => {
        ...
        /** Filter events */
        EventBus_1.StudioEventBus.subscribe(FiltersPanel_1.FilterPanelEvents.FILTER_APPLY, (event, filter) => {
...
(function (BoardEvents) {
    BoardEvents["MOUSE_DOWN"] = "board:mouse-clicked";
})(BoardEvents = exports.BoardEvents || (exports.BoardEvents = {}));
exports.MainBoard = new Board();
...
const EventBus_1 = __webpack_require__(1);
const LayerManager_1 = __webpack_require__(13);
const Layer_1 = __webpack_require__(19);
const $ = __webpack_require__(0);
__webpack_require__(10);
const IBusType_1 = __webpack_require__(2);
const FilterManager_1 = __webpack_require__(20);
const Manager_1 = __webpack_require__(3);
const Filter_1 = __webpack_require__(27);
class LayersPanel {
    constructor() {
        this.layersDom = new Map();
    }
    addLayersHolder() {
    ...
    addTopControls() {
        ...
        const addLayerControl = $("<i/>", { class: 'addIcon' });
        addLayerControl.click(e => {
            LayerManager_1.LayerManager.singleton().add(new Layer_1.Layer("Untitled Layer"));
        });
        const addFilterControl = $("<i/>", { class: 'recordIcon' });
        addFilterControl.click(e => {
            // TODO create filter
            const layer = LayerManager_1.LayerManager.singleton().getSelected();
            if (layer !== null) {
                const filter = new Filter_1.Filter(layer.getName());
                filter.updateContent(layer.getImageSrc());
                FilterManager_1.FilterManager.singleton().add(filter);
            }
        });
        const deleteControl = $("<i/>", { class: 'deleteIcon' });
        deleteControl.click(e => {
            ...
            if (layer !== null) {
                ...
                if (id !== null)
                    LayerManager_1.LayerManager.singleton().delete(id);
            }
        });
    ...
    addLayerControls() {
        const controlHolder = $("<div/>", { class: 'controls' });
        const renameLayerControl = $("<i/>", { class: 'renameIcon' });
        renameLayerControl.click(e => {
            this.onRenameHandler($(e.target).parent().parent());
        });
        const visibilityControl = $("<i/>", { class: 'visibilityIcon', 'data-visible': true });
        visibilityControl.click(e => {
            const id = $(e.target).parent().parent().data('id');
            const visible = visibilityControl.data('visible') === true;
            try {
                const layer = LayerManager_1.LayerManager.singleton().setVisibility(id, !visible);
    ...
    addLayerAlphaControl() {
        const layerAlphaGroup = $("<div/>", { class: 'swatch-group' });
        /** Layer minimum alpha icon */
        const alphaSliderMinInfo = $("<span/>", { class: 'slider-min', text: '0' });
        /** Layer Size slider */
        const alphaSlider = $("<input/>", {
            type: 'range',
            min: 0,
            max: 1,
            step: 0.01,
        ...
        alphaSlider.on("input", function (e) {
            ...
            if (layer !== null) {
                const id = layer.getId();
                if (id !== null) {
                    LayerManager_1.LayerManager.singleton().setAlpha(id, parseFloat(alphaSlider.val() || '1'));
                }
            }
        });
        EventBus_1.StudioEventBus.subscribe(LayersPanelEvents.LAYER_ALPHA_RESET, (event, data) => {
            const layer = LayerManager_1.LayerManager.singleton().getSelected();
        ...
        const alphaSliderMaxInfo = $("<span/>", { class: 'slider-max', text: '100' });
    ...
    onRenameHandler(parent) {
        try {
            const id = parent.data('id');
            const name = parent.children().first().text();
            const layer = LayerManager_1.LayerManager.singleton().get(id);
            const nameInput = $("<input/>", { type: 'text', value: name, class: 'layer-input' });
            nameInput.on('blur', e => {
                layer.rename(nameInput.val() || "Undefined");
                LayerManager_1.LayerManager.singleton().update(layer);
                nameInput.remove();
    ...
    addLayerAtEnd(id, name) {
        const layer = $("<div/>", { 'data-id': id, class: 'layer' });
        const layerTitle = $("<span/>", { html: name });
        layerTitle.click(e => {
            LayerManager_1.LayerManager.singleton().setSelected(id);
        });
        ...
        EventBus_1.StudioEventBus.publish(LayersPanelEvents.LAYER_ADDED, { id: id });
        LayerManager_1.LayerManager.singleton().setSelected(id);
    }
    addLayerAtStart(id, name) {
        const layer = $("<div/>", { 'data-id': id, class: 'layer' });
        const layerTitle = $("<span/>", { html: name });
        layerTitle.click(e => {
            LayerManager_1.LayerManager.singleton().setSelected(id);
    ...
    deleteLayer(id) {
        if (this.layersDom.has(id)) {
            ...
            if (layerToMatch !== undefined && layerToMatch === this.selectedLayer) {
                ...
                LayerManager_1.LayerManager.singleton().setSelected(i);
            }
    ...
    getFirstKeyExcept(key, obj) {
        const keys = obj.keys();
        let firstKey = keys.next();
        if (firstKey.done)
            return null;
        if (firstKey.value == key)
            firstKey = keys.next();
        if (firstKey.done)
            return null;
    ...
    initialize(panel) {
        ...
        EventBus_1.StudioEventBus.subscribe(IBusType_1.IBusType.LAYER.toString() + Manager_1.ManagerEvents.ADD.toString(), (event, data) => {
            const _id = data.getId();
            if (_id !== null)
                this.addLayerAtStart(_id, data.getName());
        });
        EventBus_1.StudioEventBus.subscribe(IBusType_1.IBusType.LAYER.toString() + Manager_1.ManagerEvents.UPDATE.toString(), (event, data) => {
            const _id = data.getId();
            if (_id !== null)
                this.renameLayer(_id, data.getName());
        });
        EventBus_1.StudioEventBus.subscribe(IBusType_1.IBusType.LAYER.toString() + Manager_1.ManagerEvents.REMOVE.toString(), (event, data) => {
            const _id = data.getId();
            if (_id !== null)
        ...
        /** Local events */
        EventBus_1.StudioEventBus.subscribe(LayerManager_1.LayerManagerEvents.LAYER_SELECTED, (event, data) => {
            if (data == null) {
                EventBus_1.StudioEventBus.publish(LayersPanelEvents.LAYER_SELECTED, null);
                return;
            }
            ;
            const id = data.getId();
            if (id !== null) {
                this.selectLayer(id);
                EventBus_1.StudioEventBus.publish(LayersPanelEvents.LAYER_SELECTED, data);
                EventBus_1.StudioEventBus.publish(LayersPanelEvents.LAYER_ALPHA_RESET, data);
            }
        });
    }
}
exports.LayersPanel = LayersPanel;
var LayersPanelEvents;
(function (LayersPanelEvents) {
    LayersPanelEvents["LAYER_ADDED"] = "layerpanel:layer-added";
    LayersPanelEvents["LAYER_SELECTED"] = "layerpanel:layer-selected";
    LayersPanelEvents["LAYER_ALPHA_RESET"] = "layerpanel:layer-alpha-reset";
})(LayersPanelEvents = exports.LayersPanelEvents || (exports.LayersPanelEvents = {}));
exports.LayersPanelModule = new LayersPanel();
...
class Layer extends EventMessageItem_1.EventMessageItem {
    constructor(title) {
        super();
    ...
    update(canvas, context, width, height) {
        this.content = context.getImageData(0, 0, width, height);
        this.imageSrc = canvas.toDataURL('image/png');
        this.image = $("<img/>", { src: this.imageSrc }).get(0);
    }
    get() {
        return this.content;
    }
    ...
    applyDrawing(context, width, height) {
        if (this.image != null) {
...
const Manager_1 = __webpack_require__(3);
const IBusType_1 = __webpack_require__(2);
class FilterManager extends Manager_1.Manager {
    constructor() {
        super(...arguments);
        this.selectedFilter = null;
    }
    bootstrap() {
        super.bootstrap(IBusType_1.IBusType.FILTER);
    }
    static singleton() {
        if (this.self === null) {
    ...
    setSelected(id) {
        try {
            ...
            EventBus_1.StudioEventBus.publish(FilterManagerEvents.FILTER_SELECTED.toString(), this.selectedFilter.clone());
        }
        catch (e) {
            throw e;
        }
...
FilterManager.self = null;
exports.FilterManager = FilterManager;
var FilterManagerEvents;
(function (FilterManagerEvents) {
    FilterManagerEvents["FILTER_SELECTED"] = "filtermanager:filter-selected";
})(FilterManagerEvents = exports.FilterManagerEvents || (exports.FilterManagerEvents = {}));
...
class FiltersPanel {
    ...
    addTopControls() {
        ...
        applyControl.click(e => {
            ...
            if (this.selectedFilter) {
                const id = this.selectedFilter.data('id');
                const fil = FilterManager_1.FilterManager.singleton().get(id);
                EventBus_1.StudioEventBus.publish(FilterPanelEvents.FILTER_APPLY, fil);
            }
        });
        ...
        deleteControl.click(e => {
            ...
            if (this.selectedFilter) {
                const id = this.selectedFilter.data('id');
                FilterManager_1.FilterManager.singleton().delete(id);
            }
        });
    ...
    onRenameHandler(parent) {
        const id = parent.data('id');
        const name = parent.children().first().text();
        const filter = FilterManager_1.FilterManager.singleton().get(id.toString());
        const nameInput = $("<input/>", { type: 'text', value: name, class: 'layer-input' });
        nameInput.on('blur', e => {
            // TODO rename filter from manager
            filter.rename(nameInput.val() || "");
            FilterManager_1.FilterManager.singleton().update(filter);
            nameInput.remove();
            parent.children().first().removeClass("hidden");
        });
        nameInput.on('keyup', e => {
    ...
    addFiltersAtEnd(id, name) {
        const filter = $("<div/>", { 'data-id': id, class: 'layer' });
        const filterTitle = $("<span/>", { html: name });
        filter.click(e => {
            FilterManager_1.FilterManager.singleton().setSelected(id);
        });
        filter.dblclick(e => {
    ...
    initialize(panel) {
        ...
        EventBus_1.StudioEventBus.subscribe(IBusType_1.IBusType.FILTER.toString() + Manager_1.ManagerEvents.ADD.toString(), (event, data) => {
            const _id = data.getId();
            if (_id !== null)
                this.addFiltersAtEnd(_id, data.getName());
            else
                throw new InvalidArgException_1.InvalidArgException("added filter has id of null");
        });
        EventBus_1.StudioEventBus.subscribe(IBusType_1.IBusType.FILTER.toString() + Manager_1.ManagerEvents.UPDATE.toString(), (event, data) => {
            const _id = data.getId();
            if (_id !== null)
                this.renameFilter(_id, data.getName());
            else
                throw new InvalidArgException_1.InvalidArgException("updated filter has id of null");
        });
        EventBus_1.StudioEventBus.subscribe(IBusType_1.IBusType.FILTER.toString() + Manager_1.ManagerEvents.REMOVE.toString(), (event, data) => {
            const _id = data.getId();
            if (_id !== null)
                this.deleteFilter(_id);
            else
                throw new InvalidArgException_1.InvalidArgException("deleted filter has id of null");
        });
        this.addTopControls();
        /** my events */
        EventBus_1.StudioEventBus.subscribe(FilterManager_1.FilterManagerEvents.FILTER_SELECTED.toString(), (event, data) => {
            const _id = data.getId();
            if (_id !== null)
                this.selectFilter(_id);
            else
                throw new InvalidArgException_1.InvalidArgException("selected filter has id of null");
        });
    }
}
exports.FiltersPanel = FiltersPanel;
var FilterPanelEvents;
(function (FilterPanelEvents) {
    FilterPanelEvents["FILTER_APPLY"] = "filterpanel:filter-apply";
})(FilterPanelEvents = exports.FilterPanelEvents || (exports.FilterPanelEvents = {}));
exports.FiltersPanelModule = new FiltersPanel();
...
Object.defineProperty(exports, "__esModule", { value: true });
const EventMessageItem_1 = __webpack_require__(9);
class Tool extends EventMessageItem_1.EventMessageItem {
}
exports.Tool = Tool;
...
Object.defineProperty(exports, "__esModule", { value: true });
const BrushLoader_1 = __webpack_require__(24);
const ColorLoader_1 = __webpack_require__(12);
const buttons_1 = __webpack_require__(25);
const Board_1 = __webpack_require__(17);
const LayerManager_1 = __webpack_require__(13);
const Layer_1 = __webpack_require__(19);
const $ = __webpack_require__(0);
const FiltersPanel_1 = __webpack_require__(21);
...
const EventBusManager_1 = __webpack_require__(32);
const EventBus_1 = __webpack_require__(1);
const BrushManager_1 = __webpack_require__(11);
const BrushTool_1 = __webpack_require__(33);
try {
    /** Loader Started */
    const loaderHolder = $("#loaderHolder");
    /**********************EventBus*****************/
    EventBusManager_1.EventBusManager.singleton().add(EventBus_1.StudioEventBus);
    EventBusManager_1.EventBusManager.singleton().add(EventBus_1.LayerEventBus);
    EventBusManager_1.EventBusManager.singleton().add(EventBus_1.ToolEventBus);
    EventBusManager_1.EventBusManager.singleton().add(EventBus_1.FilterEventBus);
    /**********************Brushes*****************/
    const brushElement = $("#brushes");
    BrushLoader_1.BrushLoaderModule.bootstrap(brushElement);
    /**********************COLORS*****************/
    const colorElement = $("#colors");
    /** Add Colors */
    const loadRandomColors = () => {
        const colorLimit = 50;
        for (let i = 0; i < colorLimit; i++) {
            ColorLoader_1.ColorLoaderModule.addColor(Helper_1.getRandomColor());
        }
    };
    loadRandomColors();
    ColorLoader_1.ColorLoaderModule.bootstrap(colorElement);
    /**********************SAVING IMAGE *******************/
    const btnElement = $("#btns");
    buttons_1.DownloadBtn.draw(btnElement);
    /**********************Drawing****************/
    const canvasHolder = $("#canvas_holder");
    Board_1.MainBoard.bootstrap(canvasHolder);
    Board_1.MainBoard.resize(window.innerWidth, window.innerHeight - 50);
    /** Panels */
    LayersPanel_1.LayersPanelModule.initialize(Panel_1.LayerFilteWindow);
    FiltersPanel_1.FiltersPanelModule.initialize(Panel_1.LayerFilteWindow);
    ToolsPanel_1.ToolsPanelModule.initialize(Panel_1.ToolsWindow);
    /**********************Layers**************************/
    LayerManager_1.LayerManager.singleton().add(new Layer_1.Layer("background"));
    /**********************TOOLS*****************/
    const brushTool = new BrushTool_1.BrushTool();
    const paintTool = new PaintTool_1.PaintTool();
    ToolManager_1.ToolManager.singleton().add(brushTool);
    ToolManager_1.ToolManager.singleton().add(paintTool);
    ToolManager_1.ToolManager.singleton().setSelected(brushTool.getId());
    /**********************BRUSHES*****************/
    BrushManager_1.BrushManager.singleton().add(Brush_1.RoundBrush);
    BrushManager_1.BrushManager.singleton().add(Brush_1.SquareBrush);
    BrushManager_1.BrushManager.singleton().add(Brush_1.TriangleBrush);
    BrushManager_1.BrushManager.singleton().add(Brush_1.ClearBrush);
    BrushManager_1.BrushManager.singleton().add(Brush_1.FadedRoundBrush);
    BrushManager_1.BrushManager.singleton().add(Brush_1.DottedRoundBrush);
    BrushManager_1.BrushManager.singleton().add(Brush_1.RandomJitterBrush);
    BrushManager_1.BrushManager.singleton().setSelected(Brush_1.RoundBrush.getId());
    /** Loader Finished */
    loaderHolder.remove();
}
catch (e) {
...
const IBusType_1 = __webpack_require__(2);
const Manager_1 = __webpack_require__(3);
class BrushLoader extends ControlSwatch_1.ControlSwatch {
    constructor() {
        super(...arguments);
        this.brushIconSize = 50;
    }
    updateView() {
        this.updateDetails();
        this.toggleSwatchView(false);
    }
    drawBrushGroup(title) {
        const brushGroup = $("<div/>", {
            class: 'swatch-group'
        });
        const titleSpan = $("<span/>", {
            class: 'group-title',
        ...
        const brushCanvas = $("<canvas/>");
        const brushContext = brushCanvas.get(0).getContext('2d');
        brushCanvas.get(0).width = this.brushIconSize;
        brushCanvas.get(0).height = this.brushIconSize;
        BrushManager_1.BrushManager.singleton().getAllBrushes((brush, i) => {
            if (brushContext !== null) {
                brushContext.clearRect(0, 0, this.brushIconSize, this.brushIconSize);
                brush.draw(null, new Position_1.Position(this.brushIconSize / 2, this.brushIconSize / 2), this.brushIconSize / 2, new Color_1.Color(0, 0, 0, 1), brushContext);
                const brushImageSource = brushCanvas.get(0).toDataURL('image/png');
                const brushImageElement = $("<img/>", {
                ...
                brushImageElement.on("click", (ev) => {
                    const pos = $(ev.target).data('_id');
                    BrushManager_1.BrushManager.singleton().setSelected(pos);
                });
    ...
    drawBrushSizeGroup(title) {
        ...
        const sizeSlider = $("<input/>", {
            ...
            max: 100,
            step: 1,
            value: BrushManager_1.BrushManager.singleton().getSize()
        });
        sizeSlider.on("input", (e) => {
            const brushSize = parseInt($(e.target).val() + '' || '10');
            BrushManager_1.BrushManager.singleton().setSize(brushSize);
        });
        /** Brush maximum size icon */
        const sizeSliderMaxInfo = $("<span/>");
    ...
    drawDetailItem() {
        /** Selected Brush details */
        const selectedBrush = BrushManager_1.BrushManager.singleton().getSelected();
        const selectedBrushDetail = $("<span/>", {
            class: 'bordered',
    ...
    bootstrap(parent) {
        this.draw(parent);
        EventBus_1.StudioEventBus.subscribe(IBusType_1.IBusType.BRUSH.toString() + Manager_1.ManagerEvents.ADD.toString(), (event, data) => {
            this.updateSwatches();
        });
        EventBus_1.StudioEventBus.subscribe(IBusType_1.IBusType.BRUSH.toString() + Manager_1.ManagerEvents.UPDATE.toString(), (event, data) => {
            this.updateSwatches();
        });
        EventBus_1.StudioEventBus.subscribe(IBusType_1.IBusType.BRUSH.toString() + Manager_1.ManagerEvents.REMOVE.toString(), (event, data) => {
            this.updateSwatches();
        });
        EventBus_1.StudioEventBus.subscribe(BrushManager_1.BrushManagerEvents.BRUSH_SELECTED, (event, data) => {
            this.updateView();
        });
    }
}
exports.BrushLoader = BrushLoader;
exports.BrushLoaderModule = new BrushLoader();
...
Object.defineProperty(exports, "__esModule", { value: true });
const Swatch_1 = __webpack_require__(26);
const Board_1 = __webpack_require__(17);
const $ = __webpack_require__(0);
exports.DownloadBtn = new Swatch_1.Swatch(() => {
    const btn = $("<a/>", {
        class: 'btn downloadbtn'
    });
    btn.click((ev) => {
        const dt = Board_1.MainBoard.getImage();
...
const $ = __webpack_require__(0);
const EventMessageItem_1 = __webpack_require__(9);
class Filter extends EventMessageItem_1.EventMessageItem {
    constructor(name) {
        super();
        this.content = null;
        this.name = name;
...
exports.Panel = Panel;
exports.LayerFilteWindow = new Panel("layer_filter");
exports.LayerFilteWindow.initiaize($("#panels"));
exports.LayerFilteWindow.resize(300, 500);
exports.LayerFilteWindow.moveTo(window.innerWidth - 300, 100);
exports.ToolsWindow = new Panel("tools");
exports.ToolsWindow.initiaize($("#panels"));
exports.ToolsWindow.resize(undefined, undefined);
exports.ToolsWindow.moveTo(0, 100);
...
class Brush extends EventMessageItem_1.EventMessageItem {
    ...
    draw(lastPosition, position, size, color, context) {
        this.setup(context);
        ...
        if (lastPosition !== null && lastPosition !== undefined) {
            Helper_1.PointsOnLineBetween(lastPosition, position, size / 4)
                .map(point => {
...
exports.Brush = Brush;
/** Brushes */
exports.RoundBrush = new Brush("Round Brush", null, (lastPosition, position, size, color, context) => {
    context.fillStyle = color.toString();
    context.arc(position.x, position.y, size, 0, 2 * Math.PI);
    context.fill();
});
exports.SquareBrush = new Brush("Square Brush", null, (lastPosition, position, size, color, context) => {
    context.fillStyle = color.toString();
    context.rect(position.x - size, position.y - size, size * 2, size * 2);
    context.fill();
});
exports.TriangleBrush = new Brush("Triangle Brush", null, (lastPosition, position, size, color, context) => {
    context.fillStyle = color.toString();
    context.beginPath();
...
exports.ClearBrush = new Brush("Clear Brush", (context) => {
    context.globalCompositeOperation = 'destination-out';
}, (lastPosition, position, size, color, context) => {
    context.arc(position.x, position.y, size, 0, 2 * Math.PI);
    context.fill();
});
exports.FadedRoundBrush = new Brush("Faded Round Brush", null, (lastPosition, position, size, color, context) => {
    ...
    alpha_radius.forEach((val, pos) => {
        const radius = Helper_1.RangeMap(pos, 0, alpha_radius.length - 1, 0, size);
        const _color = color.clone();
        _color.Alpha = Helper_1.RangeMap(val, 0, 1, 0, _color.Alpha);
...
exports.DottedRoundBrush = new Brush("Dotted Round Brush", null, (lastPosition, position, size, color, context) => {
    const _radius = [0.7, 0.3, 0.2, 0.2, 0.15];
    const _positions = [
        new Position_1.Position(position.x + 0.3 * size, position.y + 0.3 * size),
        new Position_1.Position(position.x - 0.7 * size, position.y - 0.7 * size),
        new Position_1.Position(position.x + 0.7 * size, position.y - 0.7 * size),
        new Position_1.Position(position.x - 0.7 * size, position.y),
        new Position_1.Position(position.x - 0.5 * size, position.y + 0.7 * size)
    ];
    /**
     * create circles with larger radius and smaller alpha first
     * so size of circles keeps decreasing and alpha keeps increasing
...
exports.RandomJitterBrush = new Brush("Random Jitter Brush", null, (lastPosition, position, size, color, context) => {
    const particals = 5;
    const _radius = size / 5;
    const _positions = [];
    for (let pos = 0; pos < particals; pos++) {
        const partical = new Position_1.Position(Helper_1.getRandomNumber(position.x - size, position.x + size), Helper_1.getRandomNumber(position.y - size, position.y + size));
     ...
     */
    _positions.forEach((val, pos) => {
        const radius = Helper_1.getRandomNumber(1, _radius);
        context.fillStyle = color.toString();
        context.arc(val.x, val.y, radius, 0, 2 * Math.PI);
        context.fill();
        context.beginPath();
...
class ToolsPanel {
    constructor() {
        this.toolsDom = new Map();
        this.selectedTool = null;
    }
    initialize(panel) {
        this.panel = panel;
        this.toolHolder = this.panel.addSubPanel("Tools");
        this.eventBus = EventBus_1.StudioEventBus;
        this.eventBus.subscribe(IBusType_1.IBusType.TOOL.toString() + Manager_1.ManagerEvents.ADD.toString(), (event, data) => {
            this.addTool(data);
        });
        this.eventBus.subscribe(IBusType_1.IBusType.TOOL.toString() + Manager_1.ManagerEvents.UPDATE.toString(), (event, data) => {
            this.updateTool(data);
        });
        this.eventBus.subscribe(IBusType_1.IBusType.TOOL.toString() + Manager_1.ManagerEvents.REMOVE.toString(), (event, data) => {
            this.removeTool(data);
        });
        /** my events */
        EventBus_1.StudioEventBus.subscribe(ToolManager_1.ToolManagerEvents.TOOL_SELECTED, (event, tool) => {
            if (tool === null) {
                ...
                if (_id === null) {
                    throw new InvalidArgException_1.InvalidArgException("Invalid tool selected");
                }
                ...
                if (Dtool !== undefined) {
                    ...
                    throw new InvalidArgException_1.InvalidArgException(`Tool with id ${_id} doesn't exists in DOM`);
            }
        });
    }
    addTool(tool) {
        const _id = tool.getId();
        if (_id == null)
            throw new InvalidArgException_1.InvalidArgException("Tool has invalid id of null");
        const Dtool = $("<section/>", { 'data-id': _id, class: 'tool' });
        Dtool.click(e => {
            ToolManager_1.ToolManager.singleton().setSelected(_id);
        });
    ...
    removeTool(tool) {
        ...
        if (Dtool == undefined)
            throw new InvalidArgException_1.InvalidArgException(`Tool with id ${_id} doesn't exists in DOM`);
        Dtool.empty();
        ToolManager_1.ToolManager.singleton().setSelected(null);
    }
}
exports.ToolsPanel = ToolsPanel;
var ToolsPanelEvents;
(function (ToolsPanelEvents) {
})(ToolsPanelEvents = exports.ToolsPanelEvents || (exports.ToolsPanelEvents = {}));
exports.ToolsPanelModule = new ToolsPanel();
...
class PaintTool extends Tool_1.Tool {
    constructor() {
        super();
        this.toolIcon = "https://storage.googleapis.com/material-icons/external-assets/v4/icons/svg/ic_format_paint_white_48px.svg";
        this.running = false;
    }
    initialize() {
        // do nothing
    ...
    draw(position, color, context, width, height) {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.running)
                return;
            this.running = true;
            ...
            try {
                const msg = yield Helper_1.CreateWorker('./dist/boundingbox.js', {
                    imageData,
                    position,
                    width,
                    color,
...
class EventBusManager {
    ...
    add(eventBus) {
        if (eventBus instanceof EventBus_1.EventBus) {
            const uuid = eventBus.getId();
            if (uuid !== null && !this.eventBusMap.has(uuid)) {
                this.eventBusMap.set(uuid, eventBus.clone());
            }
            else {
                throw new InvalidArgException_1.InvalidArgException("Invalid eventbus provided");
            }
        }
        else {
            throw new InvalidArgException_1.InvalidArgException("argument must be of type EventBus");
        }
    }
    update(eventBus) {
        if (eventBus instanceof EventBus_1.EventBus) {
    ...
    delete(uuid) {
        const eventBus = this.eventBusMap.get(uuid);
        if (eventBus !== undefined) {
            this.eventBusMap.delete(uuid);
            return eventBus;
        }
        else {
            throw new ItemNotFoundException_1.ItemNotFoundException(`EventBus with id ${uuid} doesn't exists`);
        }
    }
    get(uuid) {
        const eventBus = this.eventBusMap.get(uuid);
...
const BrushManager_1 = __webpack_require__(11);
const $ = __webpack_require__(0);
class BrushTool extends Tool_1.Tool {
    constructor() {
        super();
        this.toolIcon = "https://storage.googleapis.com/material-icons/external-assets/v4/icons/svg/ic_brush_white_48px.svg";
    }
    initialize() {
        // do nothing
    }
    reset() {
        ...
        BrushManager_1.BrushManager.singleton().reset();
    }
    draw(position, color, context, width, height) {
        return __awaiter(this, void 0, void 0, function* () {
            /** Draw brush */
            BrushManager_1.BrushManager.singleton().drawBrush(position, color, context);
        });
    }
    getLabel() {
        return "Brush Tool";
